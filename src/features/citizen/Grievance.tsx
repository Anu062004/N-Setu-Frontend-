  ...
  if (result) {
    return (
      <div className="intake-result" role="status">
        <StatusLabel label={result.status} />
        <h1 className="h-section mt-4">Grievance filed</h1>
        <p className="small mt-4" style={{ maxWidth: 520 }}>
          {result.id} · {result.summary} — the grievance pipeline starts here. The platform
          packages a clean evidence trail for the statutory body that owns the outcome.
        </p>
        <div className="intake-result__actions mt-6">
          <Link to="/portal" className="btn btn--primary">Back to my portal</Link>
          <Link to="/how-it-works" className="btn btn--ghost">How grievances work →</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="grievance">
      <div className="container-narrow">
        <p className="eyebrow">Citizen grievance</p>
        <h1 className="h-section">File a grievance</h1>
        <p className="small mt-3" style={{ maxWidth: 560 }}>
          Objective, platform-observable issues — a quote not honoured, a no-show, a fee not
          disclosed before work, a payment demanded on a legal-aid matter. The platform packages
          evidence; it does not adjudicate.
        </p>
        ...
        >
          <div className="field">
            <label className="field__label" htmlFor="summary">What happened?</label>
            <textarea
              id="summary"
              className="field__input"
              rows={4}
              placeholder="Describe the issue in plain language — facts only, no legal jargon required."
              value={summary}
              onChange={(e) => setSummary(e.target.value)}
            />
            <p className="field__hint">Facts only. This text is stored for the evidence trail.</p>
          </div>
            ...
            <div className="field">
              ...
              >
                {(Object.keys(CATEGORY_LABELS) as TaxCategory[]).map((t) => (
                  <option key={t} value={t}>{CATEGORY_LABELS[t]}</option>
                ))}
              </select>
            </div>
            <div className="field">
          ...
          <div className="field mt-4">
            <label className="field__label" htmlFor="related">Related booking or matter (optional)</label>
            <input
              id="related"
              className="field__input"
              placeholder="e.g. bk_4f2a1c or m_1055"
              value={related}
        ...
        <div className="credential-leg mt-6">
          <div className="credential-leg__head">
            <span className="h-micro">The pipeline</span>
          </div>
          <p className="small mt-3">{PIPELINE.join(" → ")}</p>
          <p className="small mt-3">
            Professional misconduct is a State Bar Council matter (s.35, Advocates Act 1961). The
            platform tracks the outcome so institutions see it — it never publishes verdicts.
          </p>
        </div>
      </div>
    </div>
